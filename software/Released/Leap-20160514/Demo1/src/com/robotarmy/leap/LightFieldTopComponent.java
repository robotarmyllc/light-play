...
public final class LightFieldTopComponent extends TopComponent implements ActionListener {
    ...
    public void leapConnected(Controller controller) {
        io.getOut().println("[LightField] LEAP Connected.");
        leapConnected = true;
    }

    public void leapDisconnected(Controller controller) {
        leapConnected = false;
        io.getOut().println("[LightField] LEAP Disconnected.");
    }
        ...
        if (!frame.hands().isEmpty()) {
            ...
            while (hands.hasNext()) {
                ...
                if (h.isLeft() && !leftDone) {
                    ...
                    grid.brushXY((int)(h.palmPosition().getX()/10) + (Grid.FWIDTH/2), 
                            (int)(h.palmPosition().getZ()/10) + (Grid.FHEIGHT/2), 
                            new Color(0,255-clamp((int)h.palmPosition().getY()/3), 0)
                    );
                }
                ...
                if (h.isRight() && !rightDone) {
                    ...
                    grid.brushXY((int)(h.palmPosition().getX()/10) + (Grid.FWIDTH/2), 
                            (int)(h.palmPosition().getZ()/10) + (Grid.FHEIGHT/2), 
                            new Color(0,0, 255-clamp((int)h.palmPosition().getY()/3))
                    );
