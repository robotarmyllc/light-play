...
public class DMX {
     ...
     * @param _universe Universe for device to operate on.  (not yet supported)
     * @param _mode Transmit or Receive. @DMX_TX  or @DMX_RX (not yet supported)
     * @return handle of opened port (1-MAXINT) or  -1 = Invalid _serial provided
     */
    ...
    /**
     * Set value for the @universe &  @channel.
     * 
     * @param _universe Universe to use.
     * @param _channel  Channel to set.
     * @param _data     Value to set to  (0-255)
     * @return -1 on err.  Return -2 for channel out of bounds.
     */
    native public static int setValue(int _universe, int _channel, int _data);
     ...
     */
    native public static int getValue(int _universe, int _channel);
