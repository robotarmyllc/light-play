 ...
 * ----------------------------------------------------------------------------
 */
package com.robotarmy.simpleopenni;
...
public class SimpleOpenNI extends ContextWrapper implements SimpleOpenNIConstants {
    ...
    static {   // load the nativ shared lib
        ...
        if (sysStr.indexOf("win") >= 0) {   // windows
            ...
            System.out.println("User Dit is: " + userDir);
            nativLibPath = userDir + "/Documents/Processing/libraries/SimpleOpenNI/library/";
            //nativLibPath = getLibraryPathLinux() + "/SimpleOpenNI/library/";
        ...
        try {
            ...
            System.out.println("Found jnilib for SimpleOpenNI.");
     ...
     * @param deviceIndex int
     * @param parent PApplet
     * @param runMode - RUN_MODE_DEFAULT, RunMode_SingleThreaded = Runs all in a
     * single thread - RunMode_MultiThreaded = Runs the openNI/NIITE in another
     ...
     *
     * @param parent PApplet
     * @param recordPath String, path to the record file
     * @param runMode - RUN_MODE_DEFAULT, RunMode_SingleThreaded = Runs all in a
     * single thread - RunMode_MultiThreaded = Runs the openNI/NIITE in another
     * thread than processing
     */
    ...
    protected Method getMethodRef(String methodName, Class[] paraList) {
        ...
        } catch (Exception e) { // no such method, or an error.. which is fine, just ignore
        }
        return ret;
    }
    ...
    public boolean enableRGB(int width, int height, int fps) {
        if (super.enableRGB(width, height, fps)) {	// setup the var for depth calc
            setupRGB();
            return true;
        } else {
            return false;
    ...
    public boolean enableIR(int width, int height, int fps) {
        if (super.enableIR(width, height, fps)) {	// setup the var for depth calc
            setupIR();
            return true;
        } else {
            return false;
