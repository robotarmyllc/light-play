...
// 2018 - We shift these by 3 to 're-tread' the older servos
// Servos wore a dead spot on their sensor from sitting at the same
// parking spot too long. (Orig values   44,139)
#define MIN_SERVO 35            // Was 55...  turns out these are inverted too
#define MAX_SERVO 129           // Was 140...  max is min and vice versa
                                //  so why oh why are we losing range on the output?  Hmm.
//#define FLAT_SERVO 120
// pre-2015
#define FLAT_SERVO 96     // 2018 Calibration value
//#define FLAT_SERVO 100  // 2015 Calibration value
...
/*  MJK 03/2018: Make MaxServo vary by a small random amount to
                 keep servos from wearing a groove in the position sensor.
 */
int maxServo = MAX_SERVO;  // Subtract 0-3 from maxServo in main() at some interval.
uint32_t loopCount = 0;  // Count loops so that we can change maxServo periodically.
...
// PORTC = PORTC | (1 << 4);       // Trigger scope
    ...
    led_check ();       // Check LED PWM
            ...
            phi[i]= ((phiA[i] + 40.0) * ((float)(maxServo - MIN_SERVO))
...
//PORTC = PORTC & ~(1 << 4);       // unTrigger scope
        ...
        for (i = 0; i < 3; i++) {
            ...
            if ((dmx_chan == 0) || (dmx_chan >= DMX_MAX)) {
                ...
                serv_pos[i] = phi[i];
                if (serv_pos[i] > maxServo) serv_pos[i] = maxServo; // Constrain angles
